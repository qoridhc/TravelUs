...
public class SettlementController {
  ...
  @Operation(summary = "개별 정산 요청 내역 조회", description = "개별 정산 요청 내역을 조회합니다.")
  @ApiResponses(value = {
      @ApiResponse(responseCode = "200", description = "정산 완료", content = @Content(schema = @Schema(implementation = String.class))),
      @ApiResponse(responseCode = "400", description = "잘못된 요청", content = @Content(schema = @Schema(implementation = String.class))),
      @ApiResponse(responseCode = "500", description = "서버 오류", content = @Content(schema = @Schema(implementation = String.class)))})
  @GetMapping("/personal/transfer/{userId}")
  public ResponseEntity<List<PersonalSettlementTransferResponseDto>> getPersonalSettlementHistory(@PathVariable long userId) {
    List<PersonalSettlementTransferResponseDto> response = settlementService.getPersonalSettlementHistory(userId);
    return ResponseEntity.ok().body(response);
  }
